...
public class ChargebackModel {
	...
	public List<String[]> getPlayerChargebacksByUUID(UUID player) {
		...
		try {
			ResultMap _query = this.app.getMysql().query("SELECT ws_chargeback.id, ws_chargeback.uuid, ws_chargeback.orderid FROM ws_chargeback WHERE ws_chargeback.uuid = '" + uuid + "' " + "ORDER BY " + "ws_chargeback.id DESC");
			while (_query.next()) {
				String[] result = { "", "", "" };
	...
	public int deleteChargebackByID(Integer cbid) {
		String _id = MysqlAPI.string(String.valueOf(cbid));
